...
optimized for a  use case may perform better than a Hadoop built-in data type. In such scenarios a custom 
Writable data type can be written by implementing the org.apache.hadoop.io.Writable interface to define the 
serialization format of your data type.

The Writable interface-based types can be used as value types in Hadoop MapReduce computations.

In this example, we implement a sample Hadoop Writable data type for HTTP server log
