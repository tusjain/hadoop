	...
	public void map(Object key, Text value, Context context, Node inNode) throws IOException, InterruptedException {
		...
		if (inNode.getColor() == Color.GRAY) {
			for (String neighbor : inNode.getEdges()) { // for all the adjacent nodes of the gray node
				...
				adjacentNode.setDistance(inNode.getDistance() + 1); // set the distance of the node, the distance of the adjacentNode is set to be the distance of its predecessor node+ 1, this is done since we consider a graph of unit edge weights
				adjacentNode.setColor(Color.GRAY); // set the color of the node to be GRAY
			...
			inNode.setColor(Color.BLACK);
		}

		// No matter what, emit the input node. If the node came into this method GRAY, it will be output as BLACK
