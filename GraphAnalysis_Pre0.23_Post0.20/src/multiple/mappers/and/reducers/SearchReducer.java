...
/**
 * Description : Reducer class that implements the reduce part of parallel Breadth-first search algorithm. 
 *         Make a new node which combines all information for this single node id that is for each key. The new node should have the full list of edges, the minimum distance, the darkest Color, 
...
public class SearchReducer extends Reducer<Text, Text, Text, Text> {
	...
	//the parameters are the types of the input key, the values associated with the key, the Context object through which the Reducer communicates with the Hadoop framework and the 
	//node whose information has to be output the return type is a Node
	public Node reduce(Text key, Iterable<Text> values, Context context, Node outNode) throws IOException, InterruptedException {
		...
		//since the values are of the type Iterable, iterate through the values associated with the key for all the values corresponding to a particular node id
